...
\cmd{$n$g; $n$g, }{go to or edit $n$ older/newer position in change list}
...
\cmd{:g/pattern/d $x$\\:let @+ = @a} {delete lines with pattern $c$ into
Register $x$ and copy to clipboard (\tt{:let \dots @a})}
...
\cmd{\key{esc}\or \ctrl\rbracket}{abandon edition $\to$ command mode}
...
\cmd{:$r$g/$p$/$c$ :$r$v/$p$/$c$ } {execute {\it Ex\/} $c$ on range $r$ where $p$ matches/does not match}
...
\vskip -0.2cm
\section{Ex Ranges}
\cmd{, ;\ }{separates two lines numbers, set to first line }
...
\cmd{:$r$ri $w$ } {right align lines in range $r$ to width $w$}
\cmd{:$r$le $i$ } {left align lines in range $r$ with indent $i$}
...
\cmd{:h buffers}{get help for buffer-management}
...
\cmd{:sb$n$ }{Split window and edit buffer $n$ from the buffer list}
\cmd{:vertical$n$ :horizontal$c$ }{execute $c$ on vert./horiz. split}
\cmd{:rightabove$c$ :leftbelow$c$ }{execute $c$ (eg. sb$n$) right/left above/below window}
...
\cmd{$n$\ctrl Y \ctrl E}{Scroll window $n$ lines upwards/downwards in the buffer}
...
\cmd{:set foldcolumn=4 } {show foldcolumn}
...
\cmd{g\ctrl G}{show cursor column, line, and character position}
...
% section %% search_&_replace_modifiers %{{{
\cmd{h }{Remove a trailing file name component, leaving only the head.  }
\cmd{t }{Remove all leading file name components, leaving the tail. }
\cmd{r }{Remove a trailing suffix of the form .xxx, leaving the basename. }
...
\cmd{g }{Cause  changes  to  be applied over the entire event line. }
\cmd{G }{Apply the following `s' modifier once to each word in the event line. }
% section %% search_&_replace_modifiers (end) % }}}
